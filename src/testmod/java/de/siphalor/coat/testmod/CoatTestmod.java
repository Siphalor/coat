...
public class CoatTestmod implements ClientModInitializer {
	...
	public static ConfigScreen createScreen() {
		...
		list.add(new ConfigCategoryConfigEntry<>(
				new LiteralText("A boolean with a pretty long name to demonstrate the spacing improvements"),
				new LiteralText("The checkbox needs less space, so more space for the name :)"),
		...
		ConfigCategoryWidget widget = new ConfigCategoryWidget(MinecraftClient.getInstance(), new LiteralText("Hi, my name is Fry"), list, DrawableHelper.OPTIONS_BACKGROUND_TEXTURE);
		...
		ConfigCategoryWidget widget2 = new ConfigCategoryWidget(MinecraftClient.getInstance(), new LiteralText("Ho, this is a no go"), Collections.emptyList(), DrawableHelper.OPTIONS_BACKGROUND_TEXTURE);
