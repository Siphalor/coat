...
public class ConfigListEntry<V> extends ConfigContainerCompoundEntry {
	...
	public void render(int x, int y, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
		...
		blit(x, y + 2, isDragFollow() ? 244 : 232, 0, 12, 15);
		input.render(x + 12 + CoatUtil.MARGIN, y, entryWidth - 32 - CoatUtil.DOUBLE_MARGIN, entryHeight, mouseX, mouseY, hovered, tickDelta);
		deleteWidget.x = x + entryWidth - 20;
		deleteWidget.y = y;
		deleteWidget.render(mouseX, mouseY, tickDelta);
	}
