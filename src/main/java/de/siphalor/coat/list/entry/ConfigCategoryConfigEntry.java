...
public class ConfigCategoryConfigEntry<V> extends ConfigContainerCompoundEntry implements InputChangeListener<V> {
	...
	private final BaseText originalName;
	...
	public ConfigCategoryConfigEntry(BaseText name, BaseText description, ConfigEntryHandler<V> entryHandler, ConfigInput<V> input) {
		super();
		nameWidget = new TextButtonWidget(0, 0, 100, 12, name.toString(), button -> setExpanded(!isExpanded()));
		originalName = name;
		...
		defaultButton = new ButtonWidget(0, 0, 10, 20, DEFAULT_TEXT.asFormattedString(), button ->
	...
	protected void updateExpanded(int width) {
		descriptionMultiline = textRenderer.wrapStringToWidthAsList(description.asFormattedString(), width - TEXT_INDENT);
	}
	...
	public void render(int x, int y, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
		...
		if (hovered) {
			fill(x, top, right, bottom, CoatUtil.HOVER_BG_COLOR.getArgb());
		}

		int textY = top + (int) ((inputHeight - 8) / 2F);

		input.render(x + leftInputOffset, top, inputWidth, entryHeight, mouseX, mouseY, hovered, tickDelta);
		defaultButton.y = top;
		defaultButton.x = x + entryWidth - defaultButton.getWidth() + CoatUtil.HALF_MARGIN;
		...
		if (isExpanded()) {
			...
			for (String line : descriptionMultiline) {
				textRenderer.draw(line, x + TEXT_INDENT, curY, CoatUtil.SECONDARY_TEXT_COLOR.getArgb());
				curY += 9;
			}
		}
	}
	...
	public int getBaseHeight() {
		...
		for (Message message : messages) {
			if (message.getLevel().getSeverity() >= Message.Level.DISPLAY_THRESHOLD) {
				msgHeight += textRenderer.wrapStringToWidthAsList(message.getText().asString(), parent.getEntryWidth()).size() * 9 + CoatUtil.MARGIN;
			}
		}
		if (msgHeight > 0) {
	...
	public int getExpansionHeight() {
		...
		for (Message message : messages) {
			if (message.getLevel().getSeverity() < Message.Level.DISPLAY_THRESHOLD) {
				height += textRenderer.wrapStringToWidthAsList(message.getText().asString(), parent.getEntryWidth()).size() * 9 + CoatUtil.MARGIN;
			}
		}
		return height;
	}
	...
	protected void setMessages(Collection<Message> messages) {
		...
		setName(((BaseText) originalName.copy()));
	}
